...
class SubprocVecEnv(VecEnv):
    ...
    # # generated by AI
    # def env_method_per_env(self, method_name: str, method_args_list: List) -> List[Any]:
    #     """
    #     Call instance methods of vectorized environments with different arguments for each env.
        
    #     :param method_name: Name of the method to call
    #     :param method_args_list: List of arguments to pass to each environment (one item per env)
    #     :return: List of results from each environment
    #     """
    #     assert len(method_args_list) == len(self.remotes), \
    #         f"Number of argument sets ({len(method_args_list)}) doesn't match number of environments ({len(self.remotes)})"
        
    #     # Send method calls with their specific arguments to each environment
    #     for remote, args in zip(self.remotes, method_args_list):
    #         if isinstance(args, tuple) and len(args) == 2 and isinstance(args[1], dict):
    #             # Handle case where args is (args, kwargs)
    #             args_tuple, kwargs_dict = args
    #             remote.send(("env_method", (method_name, args_tuple, kwargs_dict)))
    #         else:
    #             # Handle case where args is just positional arguments
