...
from stable_baselines3.common.buffers import DictReplayBuffer, HybridDictReplayBuffer
from stable_baselines3.common.type_aliases import DictReplayBufferSamples, HybridDictReplayBufferSamples
...
class HybridHerReplayBuffer(HybridDictReplayBuffer):
    """
    Hindsight Experience Replay (HER) buffer that support hybrid action space
    .. note::
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> DictReplayBufferSamples:  # type: ignore[override]
        ...
        actions = {
            key: th.cat((real_data.actions[key], virtual_data.actions[key]))
            for key in virtual_data.actions.keys()
        }
        dones = th.cat((real_data.dones, virtual_data.dones))
        rewards = th.cat((real_data.rewards, virtual_data.rewards))
    ...
    def _get_real_samples(
        ...
        # Prepare action dict
        actions = {key: self.to_torch(act[batch_indices, env_indices, :]) for key, act in self.actions.items()}
